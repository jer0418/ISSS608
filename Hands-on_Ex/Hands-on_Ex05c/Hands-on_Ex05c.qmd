...
In this hands-on exercise, I have gain hands-on experience on using R to plot static and interactive heatmap for visualising and analysing multivariate data.

::: callout-note
-   Heatmaps visualize data through color variations, making it easy to analyze multivariate data.

-   They help detect patterns, correlations, and relationships between variables.
:::

## **14.2 Install packages and Importing Data**
...
Noticing that the above cannot be run due to this data is a tibble not data frame therefore changing name is is deprecated

## Transforming the Data frame into a matrix

he data was loaded into a data frame, but it has to be a data matrix to make your heatmap.
...
::: callout-note
-   Different R packages provide static heatmaps, including `heatmap()`, `heatmap.2()`, `pheatmap()`, and `ComplexHeatmap`.

-   `heatmap()` is the base R function, which can cluster data by default unless disabled.
:::
...
-   `heatmap()` automatically reorders rows/columns using hierarchical clustering.

-   Normalization (scaling) improves readability by adjusting variable ranges.
:::
...
::: callout-note
-   `heatmaply` allows interactive heatmaps that can be exported as HTML.

-   The package provides improved usability and interactivity compared to static heatmaps.
:::
...
-   The function `heatmaply()` generates an interactive heatmap with automatic clustering.

-   Labels and dendrograms are placed differently than in base R’s `heatmap()`.
:::
...
::: callout-note
-   Ensures that different variables are on a comparable scale for clustering.

-   Three common methods: **scaling**, **normalization**, and **percentilizing**.
:::
...
-   Best used when variables follow a normal distribution.
:::
