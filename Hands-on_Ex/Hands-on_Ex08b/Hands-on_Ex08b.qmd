...
1.  import *SGPools_svy21.csv* into R as a tibble data frame called *sgpools*.

```{r}
sgpools <- read_csv("data/aspatial/SGPools_svy21.csv",show_col_types = FALSE)
```

2.  After importing the data file into R, it is important for us to examine if the data file has been imported correctly. The code chunk below shows list() is used to do the job.
...
```{r}
tm_shape(sgpools_sf)+ tm_bubbles(col = "red",            size = 1,            border.col = "black",            border.lwd = 1)
```

![](images/clipboard-1626441408.png)
...
![](images/clipboard-172825441.png)
...
![](images/clipboard-3104697143.png)
...
![](images/clipboard-2444625688.png)
