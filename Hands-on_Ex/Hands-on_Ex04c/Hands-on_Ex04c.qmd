...
-   **Confidence Intervals**: Use CI=xˉ±1.96×SE\text{CI} = \bar{x} \pm 1.96 \times SECI=xˉ±1.96×SE for a 95% confidence interval.

-   **Choosing the Right Visualization**: Use `geom_errorbar()` for error bars and `geom_ribbon()` for shaded confidence intervals.
:::
...
This chart displays the standard error bars of the mean math scores for different races (Chinese, Indian, Malay, Others), with the error bars representing the uncertainty range of the estimated mean.
